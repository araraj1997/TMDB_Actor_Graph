...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        if (new_edge not in self.edges) and (new_edge_reverse not in self.edges):
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for edge in self.edges:
            ...
            if edge[0] in node_degrees.keys():
                node_degrees[edge[0]] += 1
            else:
                node_degrees[edge[0]] = 1
            
            if edge[1] in node_degrees.keys():
                node_degrees[edge[1]] += 1
            else:
                node_degrees[edge[1]] = 1
        
        max_degree_value = max(node_degrees.values())
        
        max_degree_node = {k:v for k,v in node_degrees.items() if v == max_degree_value}
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        headers = {
            ...
            "Authorization": "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJjODkwYTRjMzM5MmNjOTQ3NGYzODkyYzFlMDdmZWNkNiIsInN1YiI6IjY0ZjNkNmMwOTdhNGU2MDBlMWE0MzVmNSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.IZk0zTvoM1Wt_UPYOTzql4S3fQChrlx-vzZcbQkDZdE"
        }

        # time.sleep(1)
        conn.request("GET", "/3/movie/{movie_id}/credits?language=en-US".format(movie_id = movie_id),headers = headers)
        response = conn.getresponse()
        movieid_data = response.read().decode("utf-8")  # This will return entire content.
        movieid_data_dic = json.loads(movieid_data)
        if limit and exclude_ids:
            cast_members = [cast for cast in movieid_data_dic['cast'] if (cast['order'] >= 0 and cast['order'] <= limit-1) ]
            cast_members_excluded = [cast for cast in cast_members if (int(cast['id']) not in exclude_ids)]
            return cast_members_excluded
        if limit:
            cast_members = [cast for cast in movieid_data_dic['cast'] if (cast['order'] >= 0 and cast['order'] <= limit-1) ]
            return cast_members
        if exclude_ids:
            cast_members_excluded = [cast for cast in movieid_data_dic['cast'] if (int(cast['id']) not in exclude_ids)]
            return cast_members_excluded
        
        return movieid_data_dic['cast']
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        conn.request("GET", "/3/person/{person_id}/movie_credits?language=en-US".format(person_id = person_id),headers = headers)
        response = conn.getresponse()
        movie_data = response.read().decode("utf-8")  # This will return entire content.
        movie_data_dic = json.loads(movie_data)
        # type(movie_data_dic)
        movie_credit_list = []
        
        for i in range(len(movie_data_dic['cast'])):
            
            if movie_data_dic['cast'][i]['vote_average'] >= vote_avg_threshold:
                movie_credit_list.append({key:movie_data_dic['cast'][i][key] for key in ['id', 'title', 'vote_average']})
        
        return movie_credit_list
...
if __name__ == "__main__":
    ...
    # graph.add_node(id = '123', name = 'Aravind')
    # graph.add_node(id = '23', name = 'Sreeranj')
    # graph.add_node(id = '34', name = 'Divij')
    # graph.add_edge(source = '23', target = '123')
    # graph.add_edge(source = '2975', target = '23')
    # graph.add_edge(source='34', target = '123')

    tmdb_api_utils = TMDBAPIUtils(api_key='c890a4c3392cc9474f3892c1e07fecd6')

    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK
    ####### base graph #######################

    # check_cast = tmdb_api_utils.get_movie_cast(movie_id='808', limit = 5)
    movie_credit_list = tmdb_api_utils.get_movie_credits_for_person('2975', 8.0)
    initial_nodes = []
    for movie in movie_credit_list:

        cast_members = tmdb_api_utils.get_movie_cast(movie_id=str(movie['id']), limit = 3,exclude_ids=[2975])
        
        for cast in cast_members:
            
                if (str(cast['id']), cast['name']) not in graph.nodes:
                    initial_nodes.append((str(cast['id']), cast['name']))
    ...
    for i in range(2):
        ...
        if i == 0:
            nodes_0 = initial_nodes
        else:
            # print(len(new_nodes))
            nodes_0 = new_nodes.copy()
            
        for j, node in enumerate(nodes_0):
            # print(len(nodes_0))
            # print(i,'*',j, '*', node[0])
            url = "api.themoviedb.org"
            conn = http.client.HTTPSConnection(url)
        
            movie_credit_list = tmdb_api_utils.get_movie_credits_for_person(str(node[0]), 8.0)           
            
            for movie in movie_credit_list:
                cast_members = tmdb_api_utils.get_movie_cast(movie_id=str(movie['id']), limit = 3,exclude_ids=[int(node[0])])
                # print(node[0], "original", [cast['id'] for cast in cast_members], "coactors")
                for cast in cast_members:
                    
                    if (str(cast['id']), cast['name']) not in graph.nodes:
                        new_nodes.append((str(cast['id']), cast['name']))
                
                    graph.add_node(id = str(cast['id']), name = cast['name'])
                    graph.add_edge(source = str(node[0]), target = str(cast['id']))

    
    ###### graph attributes
    # graph.print_edges()
    # graph.print_nodes()
    # print("Max degree node is ",graph.max_degree_nodes())
